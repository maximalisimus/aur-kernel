#!/bin/bash
ABSOLUT_FILENAME=$(readlink -e "$0")
filesdir=$(dirname "$ABSOLUT_FILENAME")
_sh_files=$(find "$filesdir/html" -maxdepth 1 -type f -iname "*.sh" | xargs)
_files=( $_sh_files )
unset _sh_files
for i in ${_files[*]}; do
	source $i
done
unset _files
wait
out_file="primer.html"
### Start html ###
cat ${_html_start} > ${out_file}
### Start html ###
echo -e -n "${_tab_5}${_tr_start}" >> ${out_file} ### TR
out_base_info "rstudio-desktop-git" "1.2.5033.r5032-1" "i686 x86_64" "2020-08-03" "AGPL3" # name / version / arch / date
### Debug ###
_depend=("r>=3.0.1" boost-libs qt5-sensors qt5-svg qt5-webengine qt5-xmlpatterns postgresql-libs sqlite3 soci clang hunspell-en_US mathjax2 pandoc)
### Debug ###
out_td_depend_start "${out_file}" "1" # TD: Dependens start
for i in ${_depend[*]}; do
	out_depend_part "${out_file}" "$i" # TD: a link in packages on search to aur or archlinux.org
done
out_td_depend_end "${out_file}" # TD: Dependens end
### Debug ###
_outdepend=(git subversion openssh-askpass)
### Debug ###
out_td_depend_start "${out_file}" "2" # TD: Outdependens start
for i in ${_outdepend[*]}; do
	out_depend_part "${out_file}" "$i" # TD: a link in packages on search to aur or archlinux.org
done
out_td_depend_end "${out_file}" # TD: Outdependens end
### Debug ###
_makedependens=(git cmake boost desktop-file-utils jdk8-openjdk apache-ant unzip openssl libcups pam patchelf wget yarn)
### Debug ###
out_td_depend_start "${out_file}" "3" # TD: Makedependenses start
for i in ${_makedependens[*]}; do
	out_depend_part "${out_file}" "$i" # TD: a link in packages on search to aur or archlinux.org
done
out_td_depend_end "${out_file}" # TD: Makedependenses end
_desc="A powerful and productive integrated development environment (IDE) for R programming language"
echo -e -n "${_tab_6}${_td_start}${_desc}${_td_end}\n" >> ${out_file} # TD: Description
echo -e -n "${_tab_5}${_tr_end}" >> ${out_file} ### TR
### End html ###
cat ${_html_end} >> ${out_file}
### End html ###
wait
exit 0
